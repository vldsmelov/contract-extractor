...
Локальный сервис извлечения JSON из текста договора (без табличной части). Сервис использует FastAPI, валидирует ответы по JSON Schema и обращается к внешнему демону Ollama, который должен быть запущен отдельно по адресу `http://ollama:11434`.

## Внешний сервис Ollama
- API всегда ожидает, что Ollama доступна по `http://ollama:11434`. Переменные окружения для изменения адреса больше не поддерживаются.
...
- Проверьте доступность демона командой `curl http://ollama:11434/api/tags`.

Запустить сервис Ollama можно из официального пакета (`ollama serve`) или через Docker (важно использовать host-сеть, чтобы `http://ollama:11434` был доступен из контейнера API):
...
## Сборка и запуск
1. **Убедитесь, что Ollama запущена**. Проверьте `curl http://ollama:11434/api/tags` — ответ должен содержать список моделей. Если демона нет, поднимите его командой `ollama serve` или через Docker (см. выше).

2. **Запустите сервис через Docker Compose**. Достаточно одной команды сборки и старта контейнера:
   ```bash
   docker compose up -d --build
   ```
   После старта API будет доступно на `http://localhost:8085`, Ollama — на `http://ollama:11434`.
